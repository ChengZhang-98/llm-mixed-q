...
def main():
    ...
    parser.add_argument("--stat_entry", type=str, default="threshold_count")
    ...
    df = pd.DataFrame(
        columns=["block_id", "layer", "num_outliers", "total", "ratio", "num_samples"]
    )

    for block_id in range(args.num_blocks):
        for layer_name in ["q_proj", "k_proj", "v_proj", "o_proj"]:
            new_row = [block_id, layer_name]
            for tensor_name in ["data_in"]:
                entry = (
                    f"root:model_layer_{block_id}:self_attn:{layer_name}:{tensor_name}"
                )
                new_row.append(stat_profile[entry][args.stat_entry]["num_outliers"])
                new_row.append(stat_profile[entry][args.stat_entry]["total"])
                new_row.append(
                    stat_profile[entry][args.stat_entry]["num_outliers"]
                    / stat_profile[entry][args.stat_entry]["total"]
                )
                new_row.append(stat_profile[entry][args.stat_entry]["num_samples"])
