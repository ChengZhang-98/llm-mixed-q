...
# quant_config=$work_dir/experiments/emnlp/configs/quantize/bfp_6bit.toml

# for model_name in "${MODEL_NAMES[@]}"; do
#     bash $run_dir/eval_wikitext2.sh llama $model_name $quant_config bfp_6bit
#     retVal=$?
#     if [ $retVal -ne 0 ]; then
...
for model_name in "${MODEL_NAMES[@]}"; do
    bash $run_dir/eval_wikitext2.sh llama $model_name $quant_config bfp_6bit 1 \
